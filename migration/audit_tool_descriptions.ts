...
type ToolDescriptionGroup = {
  raw_value: string;
  normalized_value: string;
  record_count: number;
  tool_category?: string;
  sample_toy_nums: string[];
};
...
async function auditToolDescriptions() {
  ...
  // Apply normalization
  const groups: ToolDescriptionGroup[] = result.rows.map(row => ({
  ...
  const normalizedMap = new Map<string, ToolDescriptionGroup[]>();
  ...
  const merges: Array<{ normalized: string; raw_values: string[]; total_records: number }> = [];
  const noChange: ToolDescriptionGroup[] = [];
  ...
  const potentialDuplicates: Array<{ value1: string; value2: string; distance: number }> = [];
  ...
  const csvLines = [
    ...
    ...groups.map(g => 
      `"${g.raw_value}","${g.normalized_value}",${g.record_count},"${g.tool_category || ''}",${g.raw_value !== g.normalized_value ? 'YES' : 'NO'},"${g.sample_toy_nums.join(', ')}"`
    ),
  ];

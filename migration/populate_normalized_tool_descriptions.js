...
async function populateNormalizedDescriptions() {
  ...
  `);
    ...
    for (let i = 0; i < result.rows.length; i += batchSize) {
        ...
        await Promise.all(batch.map(row => pool.query('UPDATE fmea_knowledge_base SET tool_description_normalized = $1 WHERE id = $2', [normalizeToolDescription(row.tool_description), row.id])));
